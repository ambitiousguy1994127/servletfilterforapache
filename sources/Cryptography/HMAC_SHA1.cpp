...
namespace idfc 
{
void CHMAC_SHA1::HMAC_SHA1(const BYTE *text, int text_len, BYTE *key, int key_len, BYTE *digest)
{
	...
	/* STEP 2 */
	for (int i=0; i<static_cast<int>(sizeof(m_ipad)); i++)
	{
		m_ipad[i] ^= SHA1_Key[i];		
	}
	...
	/* STEP 5 */
	for (int j=0; j<static_cast<int>(sizeof(m_opad)); j++)
	{
		m_opad[j] ^= SHA1_Key[j];
	}
